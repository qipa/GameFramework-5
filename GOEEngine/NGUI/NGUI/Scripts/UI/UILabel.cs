...
public class UILabel : UIWidget
{
	...
	void ProcessText (bool legacyMode, bool full)
	{
		...
		if (mPrintedSize > 0)
		{
			...
			for (int ps = mPrintedSize; ps > 0; --ps)
			{
				...
				// Wrap the text
				bool fits = NGUIText.WrapText(mText, out mProcessedText, true, IsCombined);
				...
				{
					mCalculatedSize = NGUIText.CalculatePrintedSize(mProcessedText, IsCombined);
    ...
    protected virtual bool IsCombined
    {
        get { return false; }
    }
	/// <summary>
	...
	public int GetCharacterIndexAtPosition (Vector2 localPos, bool precise)
	{
		if (isValid)
		{
            ...
            if (precise) NGUIText.PrintExactCharacterPositions(text, mTempVerts, mTempIndices, IsCombined);
            else NGUIText.PrintApproximateCharacterPositions(text, mTempVerts, mTempIndices, IsCombined);
	...
	public bool Wrap (string text, out string final, int height)
	{
		...
		bool retVal = NGUIText.WrapText(text, out final, IsCombined);
		NGUIText.bitmapFont = null;
#if DYNAMIC_FONT
		NGUIText.dynamicFont = null;
#endif
	...
	public virtual void UpdateNGUIText ()
	{
