...
static public class NGUIText
{
    ...
    static public float GetGlyphWidth(int ch, int prev, bool combined = false)
    {
        if (bitmapFont != null && !combined)
        {
    ...
    static public GlyphInfo GetGlyph(int ch, int prev, bool combined)
    {
        if (bitmapFont != null && !combined)
		{
			bool thinSpace = false;
	...
	static public Vector2 CalculatePrintedSize (string text,bool combined = false)
	{
		...
		if (!string.IsNullOrEmpty(text))
		{
			...
			for (int i = 0; i < textLength; ++i)
			{
				...
				if (symbol == null)
				{
                    float w = GetGlyphWidth(ch, prev, combined);
	...
	static public int CalculateOffsetToFit (string text, bool combined = false)
	{
    ...
    static public bool WrapText(string text, out string finalText, bool combined = false)
    {
        return WrapText(text, out finalText, false, combined);
    }
	...
	static public bool WrapText (string text, out string finalText, bool keepCharCount, bool combined = false)
	{
    ...
    static public void Print(string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, bool combined = false, bool isSymbolPass = false)
    {
        ...
        for (int i = 0; i < textLength; ++i)
        {
            ...
            {
                GlyphInfo glyph = GetGlyph(ch, prev, combined);
                ...
                {
                    GlyphInfo dash = GetGlyph(strikethrough ? '-' : '_', prev, combined);
    ...
    static public void PrintApproximateCharacterPositions(string text, BetterList<Vector3> verts, BetterList<int> indices, bool combined = false)
    {
        if (string.IsNullOrEmpty(text)) text = " ";
    ...
    static public void PrintExactCharacterPositions(string text, BetterList<Vector3> verts, BetterList<int> indices, bool combined = false)
	{
		if (string.IsNullOrEmpty(text)) text = " ";
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (symbol == null)
			{
                float gw = GetGlyphWidth(ch, prev, combined);
