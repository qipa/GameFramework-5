...
public class UIDrawCall : MonoBehaviour   
{
    ...
    static Dictionary<string, Dictionary<int, ExplictRQDCInfo>> rqInfo = new Dictionary<string, Dictionary<int, ExplictRQDCInfo>>();

    public bool IsExplictDC { get; set; }

    Material TryFetchDynamicMaterial()
    {
        if (!IsExplictDC)
            return null;
        Dictionary<int, ExplictRQDCInfo> dic;

        if (rqInfo.TryGetValue(mMaterial.mainTexture.name, out dic))
        {
            ExplictRQDCInfo info;
            if (dic.TryGetValue(renderQueue, out info))
            {
                //Debug.Log(string.Format("Got Mat:{0} Ref:{1} Valid:{2}", info.TextureName, info.References, (bool)info.Material));
                if (info.Material)
                {
                    info.References++;
    ...
    void ReleaseExplictRQMaterial()
    {
        Dictionary<int, ExplictRQDCInfo> dic;
        if (mDynamicMat == null)
            return;
        if (rqInfo.TryGetValue(mDynamicMat.mainTexture.name, out dic))
        {
            ExplictRQDCInfo info;
            if (dic.TryGetValue(renderQueue, out info))
            {
                ...
                {
                    NGUITools.DestroyImmediate(info.Material);
                    dic.Remove(renderQueue);
                }
            }
            else
	...
	void CreateMaterial ()
	{
        ...
        Material cachedMat = TryFetchDynamicMaterial();
        //if (IsExplictDC)
        //    Debug.Log("Rebuilding Material, if cachedMat found:" + (cachedMat != null));
        if (!IsExplictDC || cachedMat == null)
        {
            if (mMaterial != null)
            {
            ...
            {
                Dictionary<int, ExplictRQDCInfo> dic;
                string texName = mMaterial.mainTexture.name;
                if (!rqInfo.TryGetValue(texName, out dic))
                {
                    dic = new Dictionary<int, ExplictRQDCInfo>();
                    rqInfo[texName] = dic;
                }
                ExplictRQDCInfo info = new ExplictRQDCInfo();
                info.Material = mDynamicMat;
                info.References = 1;
                info.RQ = renderQueue;
                info.TextureName = texName;
                dic[renderQueue] = info;
            }
        }
        else
            mDynamicMat = cachedMat;
	...
	public void UpdateGeometry (int widgetCount)
	{
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			if (verts.size < 65000)
			{
                ...
                // Debug.Log(trim + "" + verts.buffer.Length + "--" + uvs.buffer.Length + "---" + cols.buffer.Length);
	...
	static public UIDrawCall Create (UIPanel panel, Material mat, Texture tex, Shader shader)
	{
		...
		return Create(string.Format("NGUI-Texture({0})",tex==null?"":tex.name), panel, mat, tex, shader);
#endif
	}
